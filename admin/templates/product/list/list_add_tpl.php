...
if (($config['product'][$type]['images_list'] == true || $config['product'][$type]['donvi_list'] == true)) {
...
function get_donvi($id = 0)
{
	...
	if ($id) {
		$temps = $d->rawQueryOne("select id_donvi from #_product_list where id = ? and type = ? limit 0,1", array($id, $type));
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="<?= $colRight ?>">
                <div id="scroll-right">
                    <?php if (isset($config['product'][$type]['donvi_list']) && $config['product'][$type]['donvi_list'] == true) { ?>
                        <div class="card card-primary card-outline text-sm">
                            <div class="card-header">
                                <h3 class="card-title">Đơn vị <?= $config['product'][$type]['title_main_list'] ?></h3>
                                <div class="card-tools">
                                    <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                                </div>
                            </div>
                            <div class="card-body">
                                <?php if (isset($config['product'][$type]['donvi_list']) && $config['product'][$type]['donvi_list'] == true) { ?>
                                    <div class="form-group">
                                        <label class="d-block" for="id_mau">Đơn vị:</label>
                                        <?= get_donvi(@$item['id']) ?>
                                    </div>
                                <?php } ?>
                            </div>
