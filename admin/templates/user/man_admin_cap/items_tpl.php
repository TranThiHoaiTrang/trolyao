<?php
    ...
    $linkAdd = "index.php?com=user&act=add_admin_cap&p=".$curPage;
	$linkEdit = "index.php?com=user&act=edit_admin_cap&p=".$curPage;
	$linkDelete = "index.php?com=user&act=delete_admin_cap&p=".$curPage;
?>
    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">Quản lý tài khoản cấp tỉnh</li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title">Danh sách tài khoản cấp tỉnh</h3>
        </div>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <?php 
                                    $temps = $d->rawQueryOne("select id_donvi from #_user where id = ? limit 0,1", array($items[$i]['id']));
                                    $donvi = $d->rawQueryOne("select tenvi from #_news where id = ? limit 0,1", array($temps['id_donvi']));
                                    ?>
                                    <a class="text-dark" href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$donvi['tenvi']?>"><?=$donvi['tenvi']?></a>
                                </td>
                                <td class="align-middle">
                                    <?php 
                                    $nhomquyen = $d->rawQueryOne("select ten from #_permission_group where id = ? limit 0,1", array($items[$i]['id_nhomquyen']));
                                    ?>
                                    <a class="text-dark" href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['email']?>"><?=$nhomquyen['ten']?></a>
                                </td>
