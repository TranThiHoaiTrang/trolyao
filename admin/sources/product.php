...
$strUrl = "";
$arrUrl = array('id_list', 'id_cat', 'id_item', 'id_sub', 'id_brand', 'id_danhmuc', 'id_danhmuc_cap');
if (isset($_POST['data'])) {
...
function get_items()
{
	...
	if (isset($_REQUEST['keyword'])) {
		...
		$tukhoa = str_replace('-', '', $func->changeTitle($tukhoa));
		$where .= " and (slugvi LIKE '%$tukhoa%' or slugvi LIKE '$tukhoa%' or slugvi LIKE '%$tukhoa')";
	}
...
function save_item()
{
	...
	if ($data) {
		...
		if (isset($config['product'][$type]['chidan']) && $config['product'][$type]['chidan'] == true) {
			if (isset($_POST['chidan_group']) && ($_POST['chidan_group'] != '')) $data['id_chidan'] = implode(",", $_POST['chidan_group']);
			else $data['id_chidan'] = "";
		}

		$item_danhmuc = $d->rawQueryOne("select tenkhongdauvi from #_product_danhmuc where id = '" . $data['id_danhmuc'] . "' and type = '$type' limit 0,1");


		$data['tenkhongdauvi'] = (isset($data['tenvi'])) ? $func->changeTitle($func->sub_str($data['tenvi'], 150)) . '-' . $item_danhmuc['tenkhongdauvi'] : '';
		$data['tenkhongdauen'] = (isset($data['tenen'])) ? $func->changeTitle($data['tenen']) : '';


		$data['slugvi'] = str_replace('-', '', $func->changeTitle($data['tenkhongdauvi']));
		$data['slugen'] = str_replace('-', '', $func->changeTitle($data['tenen']));
		...
		if ($type == 'trungtam_quanly' || $type == 'van-ban-nb' || $type == 'phonghop-khonggiay') {
			$temps_list = $d->rawQueryOne("select id_donvi from #_product_list where id = ? and type = ? limit 0,1", array($data['id_list'], $type));
			$temps = $d->rawQueryOne("select tenvi,id from #_news where id = ? and type = 'quanly_donvi' limit 0,1", array($temps_list['id_donvi']));
			$data['id_donvi'] = $temps['id'];
		}

		// var_dump($data);
		// die();
		$data['ngaybanhanh'] = (isset($data['ngaybanhanh'])) ? strtotime($data['ngaybanhanh']) : 0;
	}
	...
	if (isset($config['product'][$type]['cauhoi']) && $config['product'][$type]['cauhoi'] == true) {
		$datafaqall = [];
	...
	if (isset($_FILES['file-taptin'])) {
		...
		if ($taptin = $func->uploadImage("file-taptin", $config['product'][$type]['file_type'], 'upload/file/', $file_name)) {
			$data['taptin'] = $taptin;
			$row = $d->rawQueryOne("select id, taptin from #_product_danhmuc where type = ? limit 0,1", array($type));
...
function save_list()
{
	...
	if ($data) {
		...
		$data['tenkhongdauvi'] = (isset($data['tenvi'])) ? $func->changeTitle($data['tenvi']) . '-' . $type : '';
		$data['tenkhongdauen'] = (isset($data['tenen'])) ? $func->changeTitle($data['tenen']) : '';

		if (isset($config['product'][$type]['donvi_list']) && $config['product'][$type]['donvi_list'] == true) {
...
function save_danhmuc()
{
	...
	if ($data) {
		...
		$data['hienthi'] = (isset($data['hienthi'])) ? 1 : 0;
		$data['type'] = $type;
		$data['ngaybanhanh'] = strtotime($data['ngaybanhanh']);
		$data['ngayhethieuluc'] = strtotime($data['ngayhethieuluc']);
		// var_dump($data);die();
	}
	/* Post seo */
...
function get_danhmuc_caps()
{
	...
	$count = $d->rawQueryOne($sqlNum, array($type));
	$total = $count['num'];
	$url = "index.php?com=product&act=man_danhmuc_cap" . $strUrl . "&type=" . $type;
	$paging = $func->pagination($total, $per_page, $curPage, $url);
}
...
function save_danhmuc_cap()
{
	...
	if ($data) {
		...
		$data['tenkhongdauvi'] = (isset($data['tenvi'])) ? $func->changeTitle($data['tenvi']) . '-' . $item_danhmuc['tenkhongdauvi'] : '';
		$data['tenkhongdauen'] = (isset($data['tenen'])) ? $func->changeTitle($data['tenen']) : '';
