...
switch ($act) {
	...
	case "man_admin_cap":
		...
		get_items_admin_cap();
		$template = "user/man_admin_cap/items";
		break;
	case "add_admin_cap":
		/* Kiểm tra active user admin */
		if (!isset($config['user']['admin']) || $config['user']['admin'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
		...
		$template = "user/man_admin_cap/item_add";
		break;
	case "edit_admin_cap":
		/* Kiểm tra active user admin */
		if (!isset($config['user']['admin']) || $config['user']['admin'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
...
function get_items_admin_cap()
{
	...
	$count = $d->rawQueryOne($sqlNum);
	$total = $count['num'];
	$url = "index.php?com=user&act=man_admin_cap";
	$paging = $func->pagination($total, $per_page, $curPage, $url);
}

/* Edit admin */
function get_item_admin_cap()
{
	global $d, $func, $curPage, $item;

	$id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

	if (!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_admin_cap&p=" . $curPage, false);

	$item = $d->rawQueryOne("select * from #_user where id = ? and id!= 1 limit 0,1", array($id));

	if (!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=user&act=man_admin_cap&p=" . $curPage, false);
}

/* Save admin */
function save_item_admin_cap()
{
	global $d, $func, $curPage, $config;

	if (empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_admin_cap&p=" . $curPage, false);
	...
	if ($data) {
		...
		$data['hienthi'] = (isset($data['hienthi'])) ? 1 : 0;
		$data['cap'] = 1;
		$data['id_nhomquyen'] = 28;

		if (isset($_POST['donvi_group']) && ($_POST['donvi_group'] != '')) $data['id_donvi'] = implode(",", $_POST['donvi_group']);
		else $data['id_donvi'] = "";
	}
	// $func->dump($data); die();
	/* Kiểm tra username */
	$username = isset($data['username']) ? $data['username'] : '';
	$check_username = $d->rawQueryOne("select id from #_user where username = ? and id <> ? limit 0,1", array($username, $id));
	if (!empty($check_username)) $func->transfer("Tên đăng nhập này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man_admin_cap&p=" . $curPage, false);

	if ($id) {
		if ($func->check_permission()) {
			$row = $d->rawQueryOne("select id from #_user where id = ? limit 0,1", array($id));
			if (isset($row['id']) && $row['id'] > 0) $func->transfer("Bạn không có quyền trên tài khoản này. Mọi thắc mắc xin liên hệ quản trị website", "index.php?com=user&act=man_admin_cap&p=" . $curPage, false);
		}
		...
		if (isset($data['password']) && ($data['password'] != '')) {
			...
			if ($confirm_password == '') {
				$func->transfer("Chưa xác nhận mật khẩu mới", "index.php?com=user&act=edit_admin_cap&id=" . $id . "&p=" . $curPage, false);
			}

			if ($password != $confirm_password) {
				$func->transfer("Xác nhận mật khẩu mới không chính xác", "index.php?com=user&act=edit_admin_cap&id=" . $id . "&p=" . $curPage, false);
			}
		...
		$d->where('id', $id);
		if ($d->update('user', $data)) {
			$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=user&act=man_admin_cap&p=" . $curPage);
		} else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=user&act=man_admin_cap&p=" . $curPage, false);
	} else {
		if (isset($data['password']) && ($data['password'] == '')) $func->transfer("Chưa nhập mật khẩu", "index.php?com=user&act=man_admin_cap&p=" . $curPage, false);
		$data['password'] = md5($config['website']['secret'] . $data['password'] . $config['website']['salt']);

		if ($d->insert('user', $data)) $func->transfer("Lưu dữ liệu thành công", "index.php?com=user&act=man_admin_cap&p=" . $curPage);
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=user&act=man_admin_cap", false);
	}
}

/* Delete admin */
function delete_item_admin_cap()
{
	global $d, $func, $curPage;
	...
	if ($id) {
		$d->rawQuery("delete from #_user where id = ? and id!= 1", array($id));
		$func->transfer("Xóa dữ liệu thành công", "index.php?com=user&act=man_admin_cap&p=" . $curPage);
	} elseif (isset($_GET['listid'])) {
		$listid = explode(",", $_GET['listid']);
	...
	$func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_admin_cap&p=" . $curPage, false);
}
...
function get_items()
{
	...
	if ($_SESSION[$login_admin]['cap'] == '1') {
		$where .= ' and id_donvi = '.$_SESSION[$login_admin]['id_donvi'];
	}
...
function save_item()
{
	...
	if ($data) {
		...
		$data['ngaysinh'] = strtotime(str_replace("/", "-", $data['ngaysinh']));
		$data['hienthi'] = (isset($data['hienthi'])) ? 1 : 0;
		$data['nguoitao'] = $_SESSION[$login_admin]['username'];
		$data['id_nguoitao'] = $_SESSION[$login_admin]['id'];
		if (isset($_POST['donvi_group']) && ($_POST['donvi_group'] != '')) $data['id_donvi'] = implode(",", $_POST['donvi_group']);
		else $data['id_donvi'] = "";
	}
