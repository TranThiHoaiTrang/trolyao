...
function save_setting()
{
	...
	$url = $_POST['file_name'];
	$file_content = $data['robot'];
	
	if ($file_content !== false) {
		file_put_contents('../robots.txt', $file_content);
	}
	/* Post Seo */
	...
	if ($row['id']) {
		...
		if ($d->update('setting', $data)) {
			...
			// var_dump($d->insert('seo',$dataSeo));
			$func->redirect("index.php?com=setting&act=capnhat&type=setting");
		} else {
			$func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=setting&act=capnhat&type=setting", false);
		}
	} else {
