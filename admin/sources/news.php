...
function get_item()
{
	global $d, $strUrl, $func, $curPage, $item, $gallery, $type, $com, $act, $faqDB;
...
function save_item()
{
	...
	if ($data) {
		...
		if (isset($config['news'][$type]['donvi']) && $config['news'][$type]['donvi'] == true) {
	...
	if (isset($config['news'][$type]['cauhoi']) && $config['news'][$type]['cauhoi'] == true) {
		$datafaqall = [];
		$count = $_POST['count_faq'];
		for ($i = 0; $i <= $count; $i++) {
			// var_dump('datafaq'.$i);
			// var_dump($_POST['datafaq'.$i]);
			$datafaq_row = (isset($_POST['datafaq'.$i])) ? $_POST['datafaq'.$i] : null;
			if ($datafaq_row['tenvi'] != '') {
				foreach ($datafaq_row as $column => $value) {
