<script>
	...
	$(document).ready(function() {
		...
		$('body').on('change', '.select-alldanhmuc', function() {
			var id = $(this).val();
			var type = $(this).data('type');
			if ($("#danhmuc_cap_group").length) {
				$.ajax({
					...
					success: function(result) {
						var op = "<option value='0'>Chọn chương</option>";
						$("#danhmuc_cap_group").html(result);
					}
				});
