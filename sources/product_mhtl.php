...
if (isset($_POST['submit_dowload'])) {
	...
	$html_content = '
    <div style="font-family: Times New Roman; font-size: 14pt; line-height: 1.3;">
        ' . $noteContent . '
    </div>
    ';
	$html_content = htmlspecialchars_decode($html_content, ENT_QUOTES);
    // Thêm nội dung HTML vào section
    try {
        \PhpOffice\PhpWord\Shared\Html::addHtml($section, $html_content, false, false);
    } catch (Exception $e) {
        echo 'Lỗi khi thêm nội dung HTML: ', $e->getMessage();
        die();
    }

    // Xuất file
    $filename = "noidung_" . $time . "_" . date('d_m_Y') . ".docx";
    ...
    try {
        ...
        echo 'Lỗi khi lưu file Word: ', $e->getMessage();
    }
}
