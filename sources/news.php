<?php  
	...
	else if($idl!='')
	{
		...
		if(!empty($seoDB['keywords'.$seolang])) $seo->setSeo('keywords',$seoDB['keywords'.$seolang]);
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		if(!empty($news_list['canonical'.$seolang])) $seo->setSeo('canonical',$seoDB['canonical'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$seo->setSeo('url_canonical',$func->getPageCANONICAL($news_list['canonical'.$lang]));
	...
	else if($idc!='')
	{
		...
		if(!empty($seoDB['keywords'.$seolang])) $seo->setSeo('keywords',$seoDB['keywords'.$seolang]);
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		if(!empty($news_cat['canonical'.$seolang])) $seo->setSeo('canonical',$seoDB['canonical'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$seo->setSeo('url_canonical',$func->getPageCANONICAL($news_cat['canonical'.$lang]));
