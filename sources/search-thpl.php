...
if (isset($_GET['keyword'])) {
	...
	if ($tukhoa) {
		...
		$count_sanpham_all = $d->rawQueryOne("select count(id) as numb from #_product where type='tinh-huong-phap-ly' $where and hienthi>0 ");
        $count_sanpham_list_all = $d->rawQueryOne("select count(id) as numb from #_product_list where type='tinh-huong-phap-ly' $where and hienthi>0 ");
	}
}
