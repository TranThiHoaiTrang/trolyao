...
if (isset($_GET['keyword'])) {
	...
	if ($tukhoa) {
		...
		// var_dump($where);
		$count_sanpham_all = $d->rawQueryOne("select count(id) as numb from #_product where type='van-ban-ks' $where and hienthi>0 ");
		$count_sanpham_list_all = $d->rawQueryOne("select count(id) as numb from #_product_list where type='van-ban-ks' $where and hienthi>0 ");
	}
}
