<?php
	...
	echo '<url><loc>'.$config_base.'</loc><lastmod>'.date('c',time()).'</lastmod><changefreq>daily</changefreq><priority>1</priority></url>';
	if(isset($requick))
	{
		foreach($requick as $value)
		{
			$func->createSitemap($value['com'],$value['type'],$value['field'],$value['tbl'],"c","weekly","0.5","vi","stt,id",@$value['menu']);
		}
	}
	echo '</urlset>';
