...
class Functions
{
	...
	public function check_access($com = '', $act = '', $type = '', $array = null, $case = '', $dropdown = null)
	{
		...
		if ($case == 'phrase-1') {
			if ($type != '') $str .= '_' . $type;
			// var_dump(in_array($str, $_SESSION['list_quyen']));
	...
	public function get_rating_schema($id = 0)
	{
	...
	public function getPageCANONICAL($canonical = '')
	{
		if (!empty($canonical)) {
			$pageURL = 'http';
			if (array_key_exists('HTTPS', $_SERVER) && $_SERVER["HTTPS"] == "on") $pageURL .= "s";
			$pageURL .= "://";
			$pageURL .= $_SERVER["SERVER_NAME"] . '/' . $canonical;
			return $pageURL;
		}
	}
	...
	public function uploadImage_clien($file = '', $extension = '', $folder = '', $newname = '')
	{
		global $config;
		if (isset($_FILES[$file]) && !$_FILES[$file]['error']) {
			$postMaxSize = ini_get('post_max_size');
			...
			if (!move_uploaded_file($_FILES[$file]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . '/upload/file/' . $_FILES[$file]['name'])) {
				return false;
			}
	...
	{
		// Cắt chuỗi HTML thành độ dài giới hạn
		$chuoi_cat = mb_substr(strip_tags($chuoi), 0, $gioihan);

		// Kiểm tra xem chuỗi đã cắt có đủ dài không
		if (mb_strlen(strip_tags($chuoi)) > $gioihan) {
			// Nếu có, thêm dấu ba chấm vào cuối chuỗi
			$chuoi_cat .= '...';
		}
	...
	{
		$originalText = $text; // Lưu trữ văn bản nguyên bản
	
		// Chuẩn hóa văn bản và từ khóa sang chữ thường với hỗ trợ mã hóa đa byte
		$text = mb_strtolower($text, 'UTF-8');
		$keywordString = mb_strtolower($keywordString, 'UTF-8');
	
		// Xóa các dấu câu ở cuối chuỗi từ khóa
		$keywordString = rtrim($keywordString, " \t\n\r\0\x0B.,!?");
	
		// Tách từ khóa thành các từ đơn lẻ
		$keywords = explode(' ', $keywordString);
	
		// Tạo mảng chứa các mẫu regex từ các từ khóa, bao gồm cả từ không dấu
		$patterns = array_map(function ($keyword) {
			$keywordWithoutAccents = $this->url_title($keyword, ' ');
			return preg_quote($keyword, '/') . '|' . preg_quote($keywordWithoutAccents, '/');
		}, $keywords);
	
		// Kết hợp các mẫu lại thành một biểu thức regex
		$pattern = "/(" . implode('|', $patterns) . ")(?![^<]*>)/i";
		// Thực hiện tô đậm trên văn bản nguyên bản
		// var_dump($pattern);
		$originalText = preg_replace($pattern, '<span style="background-color: rgba(221, 140, 140, 0.3); font-weight: inherit;">$1</span>', $originalText);
	
		return $originalText; // Trả lại văn bản nguyên bản đã được tô nền
	}

	function highlightKeyword_product($text, $keywordString)
	{
		$text = mb_strtolower($text, 'UTF-8');
		$keywordString = mb_strtolower($keywordString, 'UTF-8');

		if (strpos($text, $keywordString) === false) {
			return $text;
		}
		...
		$keywords = [];
		if (str_word_count($keywordString) > 1) {
			$keywords[] = $keywordString;
		}

		$words = explode(' ', $keywordString);
		...
		foreach ($keywords as $keyword) {
			...
			$pattern = "/($keyword)(?![^<]*>)/i";
			$text = preg_replace($pattern, '<span style="background-color: yellow;">$1</span>', $text);
		}
		return $text;
	}
}
