...
$where = $id_loaivai . $id_chatlieu . $id_muavu;
if(empty($id_loaivai) && empty($id_chatlieu) && empty($id_muavu)){
	$where = $id_list . $id_cat;
}
else{
	$where = $id_loaivai . $id_chatlieu . $id_muavu;
}
...
$sanpham = $d->rawQuery("select * from #_product where type='san-pham' $id_list $id_cat and hienthi > 0 order by stt,id desc");
?>
<?php if ($product) { ?>
			...
			<a href="<?= $v['tenkhongdauvi'] ?>">
				<div class="bangvai">
					<div class="img_bangvai">
						<!-- <img src="<?= THUMBS ?>/606x764x1/<?= UPLOAD_PRODUCT_L . $v['photo'] ?>" alt="" width="306" height="386"> -->
						<?= Helper::the_thumbnail($v['photo'], 400, 400, '', $v['ten' . $lang], true) ?>
					</div>
					<div class="content_bangvai">
						<div class="name_bangvai text-split"><?= $v['ten' . $lang] ?></div>
						<?php if ($v['mota' . $lang]) : ?>
							<div class="sp-desc"><?= $v['mota' . $lang] ?></div>
						<?php endif; ?>
