<?php
$list_vb_ks = $d->rawQuery("select * FROM table_product_list WHERE hienthi > 0 AND type = 'van-ban-ks'  order by stt,id asc");
?>
<div class="fixwidth">
                    ...
                    <div class="trolyao_layout_center_boloc">
                        <div class="all_frm_timkiem">
                            <div class="frm_timkiem">
                                ...
                                <input type="text" class="input" id="keyword_vks" placeholder="Nhập từ khóa ..." onkeypress="doEnter_vbks(event,'keyword_vks');" value="<?= $all_vb_chinh['ten' . $lang] ?>">
                                <div class="all_button_search">
                                    ...
                                    <?php } ?>
                                    <button type="submit" value="" class="nut_tim" aria-label="Search" onclick="onSearch_keyword_vbks('keyword_vks');"><i class="fas fa-location-arrow"></i></button>
                                </div>
                            </div>
                        ...
                        <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                            ...
                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                                ...
                                foreach ($list_vb_ks as $item) {
                                    $count_spl = $d->rawQueryOne("select count(id) as numb from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-ks'");
                                ?>
                                    <li class="nav-item" role="presentation">
                            ...
                            <div class="tab-content" style="padding-top: 10px;">
                                <div class="tab-pane active" id="home" role="tabpanel" aria-labelledby="home-tab">
                                    <div class="all_vanban_kiemsoat">
                                        ...
                                        $loaivanban = $d->rawQuery("select * from #_product where hienthi>0 and type='van-ban-ks' order by stt,id desc $limit");

                                        $sqlNum = "select count(*) as 'num' from #_product where hienthi>0 and type='van-ban-ks' order by stt,id desc";
                                        ...
                                        foreach ($loaivanban as $v) {
                                            $loaivanban_list = $d->rawQueryOne("select * from #_product_list where id = '" . $v['id_list'] . "' and hienthi>0 and type='van-ban-ks' order by stt,id asc");
                                            $i++;
                                        ?>
                                ...
                                ?>
                                    <div class="tab-pane" id="<?= $item['id'] ?>" role="tabpanel" aria-labelledby="<?= $item['id'] ?>-tab">
                                        <div class="all_vanban_kiemsoat">
                                            ...
                                            $loaivanban = $d->rawQuery("select * from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-ks' order by stt,id desc $limit");

                                            $sqlNum = "select count(*) as 'num' from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-ks' order by stt,id desc";
