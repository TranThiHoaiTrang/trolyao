                    ...
                    <div class="trolyao_layout_center_boloc">
                        <div class="all_frm_timkiem">
                            <div class="frm_timkiem">
                                ...
                                <?php $all_vb_chinh = $d->rawQueryOne("select * from #$table_danhmuc_cacloai where id = '$id_danhmuc_cacloai' and hienthi>0 and type='van-ban-nb'"); ?>
                        ...
                        <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                            ...
                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                                ...
                                foreach ($list_vb_ks as $item) {
                                    $count_spl = $d->rawQueryOne("select count(id) as numb from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-nb'");
                                ?>
                                    <li class="nav-item" role="presentation">
                            ...
                            <div class="tab-content" style="padding-top: 10px;">
                                <div class="tab-pane active" id="home" role="tabpanel" aria-labelledby="home-tab">
                                    <div class="all_vanban_kiemsoat">
                                        ...
                                        if ($check == true) {
                                            $loaivanban = $d->rawQuery("select * from #_product where hienthi>0 and type='van-ban-nb' and id_donvi = '$id_donvi' order by stt,id desc $limit");

                                            $sqlNum = "select count(*) as 'num' from #_product where hienthi>0 and type='van-ban-nb' and id_donvi = '$id_donvi' order by stt,id desc";
                                        } else {
                                            $loaivanban = $d->rawQuery("select * from #_product where hienthi>0 and type='van-ban-nb' order by stt,id desc $limit");

                                            $sqlNum = "select count(*) as 'num' from #_product where hienthi>0 and type='van-ban-nb' order by stt,id desc";
                                        }
                                        ...
                                        // $loaivanban = $d->rawQuery("select * from #_product where hienthi>0 and type='van-ban-nb' order by stt,id asc");
                                        $i = 0;
                                ...
                                ?>
                                    <div class="tab-pane" id="<?= $item['id'] ?>" role="tabpanel" aria-labelledby="<?= $item['id'] ?>-tab">
                                        <div class="all_vanban_kiemsoat">
                                            ...
                                            if($check == true){
                                                $loaivanban = $d->rawQuery("select * from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-nb' and id_donvi = '$id_donvi' order by stt,id desc $limit");

                                                $sqlNum = "select count(*) as 'num' from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-nb' and id_donvi = '$id_donvi' order by stt,id desc";
                                            }else{
                                                $loaivanban = $d->rawQuery("select * from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-nb' order by stt,id desc $limit");

                                                $sqlNum = "select count(*) as 'num' from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-nb' order by stt,id desc";
                                            }
                                            ...
                                            $url = $func->getCurrentPageURL();
                                            $paging = $func->pagination($total, $per_page, $curPage, $url);
                                            // $loaivanban = $d->rawQuery("select * from #_product where id_list = '" . $item['id'] . "' and hienthi>0 and type='van-ban-nb' order by stt,id asc");
                                            $i = 0;
                                            foreach ($loaivanban as $v) {
                                                $loaivanban_list = $d->rawQueryOne("select * from #_product_list where id = '" . $v['id_list'] . "' and hienthi>0 and type='van-ban-nb' order by stt,id asc");
                                                $i++;
